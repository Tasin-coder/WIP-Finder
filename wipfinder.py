...
def ip_in_subnetwork(ip_address, subnetwork):
    ...
    if version1 != version2:
        raise ValueError("Sorry incompatible IP versions")
...
def ip_to_integer(ip_address):
    ...
    raise ValueError("Sorry invalid IP address")
...
def subnetwork_to_ip_range(subnetwork):
    ...
    raise ValueError("Sorry invalid subnetwork")
...
def crimeflare(target):
    print_out(Fore.CYAN + "Scanning in crimeflare database...")
    ...
    if (len(crimeFoundArray) != 0):
        ...
        print_out("Sorry did not find anything.")
...
def init(target):
    ...
    try:
        ...
        print_out(Fore.RED + "Sorry the Domain is not valid, exiting")
        sys.exit(0)
    ...
    try:
        ...
        if ifIpIsWithin:
            ...
            print_out(Fore.RED + args.target + " is not part of the Cloudflare network, quitting it...")
            sys.exit(0)
    except ValueError:
        print_out(Fore.RED + "IP address does not appear to be within Cloudflare range, shutting it down..")
        sys.exit(0)
...
def subdomain_scan(target, subdomains):
    ...
    if check_for_wildcard(target):
        ...
        print_out(Fore.CYAN + "Congrats scanning finished...")
        return
    ...
    try:
        ...
        print_out(Fore.RED + "Subdomains file does not exist in data directory, aborting the scan...")
        sys.exit(1)

def update():
    print_out(Fore.CYAN + "Just checking for updates, please wait...")
    print_out(Fore.CYAN + "Updating WIP Finder subnet...")
    if(args.tor == False):
...
print_out("Initializing WIP Finder - the date is: " + datestr)
